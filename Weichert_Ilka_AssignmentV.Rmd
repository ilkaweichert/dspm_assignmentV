...
document each logical programming step using R Studio.
...
The hyperlink to repository reads 
...
``german_event_venues``, call the object and check it's status running 
``status_code`` to make sure that everything worked fine. The status code of 200
tells me that the command has been successfully realized.
...
german_event_venues <- 
  GET(paste("https://app.ticketmaster.com/discovery/v2/venues.json?countryCode=DE&apikey=", api_key, sep = ""))
...
## even though the content does not seem to be the same
...
venue_data <-
  as.data.frame(cbind(
      german_event_venues_reduced[["_embedded"]][["venues"]][["name"]], 
      german_event_venues_reduced[["_embedded"]][["venues"]][["city"]][["name"]],
      german_event_venues_reduced[["_embedded"]][["venues"]][["postalCode"]],
      german_event_venues_reduced[["_embedded"]][["venues"]][["address"]][["line1"]],
      german_event_venues_reduced[["_embedded"]][["venues"]][["url"]],
      german_event_venues_reduced[["_embedded"]][["venues"]][["location"]][["longitude"]],
      german_event_venues_reduced[["_embedded"]][["venues"]][["location"]][["latitude"]]
      )
  )
...
n_page <- german_event_venues_reduced[["page"]][["totalPages"]]

# check number of total entries
german_event_venues_reduced[["page"]][["number"]]

# check number of entries on each page
german_event_venues_reduced[["page"]][["size"]]
...
for (i in page_number) {
  full_venue_data <-
    ...
    german_venues <-
    fromJSON(content(part_venues, as = "text"))
    
    full_venue_data[(20 * i - 19):(10 * i), ] <- list.stack(german_venues$embedded)
